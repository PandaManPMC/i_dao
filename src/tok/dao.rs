...
pub fn add(tx: &mut Transaction, m: &mut impl BaseModel)  -> Result<(), String> {
...
pub fn add_batch(tx: &mut Transaction, lst: &mut Vec<&mut impl BaseModel>)  -> Result<(), String> {
    let now = SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs();
    for val in &mut *lst {
...
pub fn update_by_pk(tx: &mut Transaction, m: &mut impl BaseModel) -> Result<() , String> {
    let now = SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs();
    m.set_updated_at(now);
...
pub fn update_by_sql(tx: &mut Transaction, sql: String, params: Params) -> Result<() , String> {
